# Vendored from the `ai-edge-litert` PyPI package (2.2.0)
# (google-ai-edge/LiteRT, successor to TensorFlow Lite; upstream schema is
# tensorflow/tensorflow's tensorflow/lite/schema/schema.fbs), Copyright
# The TensorFlow Authors / Google AI Edge Authors, licensed Apache 2.0
# (https://www.apache.org/licenses/LICENSE-2.0). Vendored rather than added
# as a runtime dependency because this generated file's own imports are
# just `flatbuffers` -- confirmed directly (`grep '^import\|^from'`) -- so
# it's usable without installing TensorFlow or any TFLite runtime, matching
# ../onnx_to_tflite_flatbuffers.py's own no-TensorFlow-at-runtime design.
# Not modified below this point; see that script for how it's used.
