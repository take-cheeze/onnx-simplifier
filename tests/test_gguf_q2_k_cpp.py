"""Tests for ``onnxsim.apply_gguf_q2_k_quantization_cpp`` -- the C++-backed
port of ``onnxsim.apply_gguf_q2_k_quantization`` (see
``onnxsim/passes/gguf_q2_k.h``). Like the other simple block-quant formats
...
(``tests/test_gguf_q6_k_cpp.py``, ``tests/test_gguf_legacy_quant_cpp.py``).
"""
...
import onnxsim
from onnxsim.gguf_q2_k import _SUB_BLOCK_SIZE, _SUPER_BLOCK_SIZE
...
def test_cpp_replaces_weight_with_same_shape_float():
    ...
    q = onnxsim.apply_gguf_q2_k_quantization_cpp(model)
...
def test_cpp_at_most_4_distinct_levels_per_16_element_sub_block():
...
def test_cpp_beats_naive_single_scale_int2_on_mixed_sub_blocks():
    # Q2_K's per-sub-block (scale, min) should beat a single super-block-
    # wide symmetric int2 scale on data whose sub-blocks have wildly
    # different magnitudes and offsets.
    ...
    for start in range(0, flat_in.size, _SUPER_BLOCK_SIZE):
        ...
        scale = max(np.abs(block).max(), 1e-12) / 3.0
        codes = np.clip(np.round(block / scale), -3, 3)
        naive_out[start : start + _SUPER_BLOCK_SIZE] = codes * scale

    q2_k_mse = float(np.mean((flat_in - new_w.reshape(-1)) ** 2))
    naive_mse = float(np.mean((flat_in - naive_out) ** 2))
    assert q2_k_mse < naive_mse


def test_cpp_worse_reconstruction_than_q6_k_on_same_weight():
    # Q2_K's own 2-bit element code is a strictly coarser budget than
    # Q6_K's own 6-bit one -- cross-checks this port against the
    # already-verified Q6_K port on the same input.
    rng = np.random.default_rng(9)
    K, N = _SUPER_BLOCK_SIZE * 2, 8
    w = rng.standard_normal((K, N)).astype(np.float32) * 0.5
    model = _matmul_model(w, K, N)

    q2 = onnxsim.apply_gguf_q2_k_quantization_cpp(model)
    q6 = onnxsim.apply_gguf_q6_k_quantization_cpp(model)
    w64 = w.astype(np.float64)
    err_q2 = np.linalg.norm(w64 - _current_weight(q2).astype(np.float64))
    err_q6 = np.linalg.norm(w64 - _current_weight(q6).astype(np.float64))
    assert err_q2 > err_q6
...
def test_cpp_behaves_similarly_to_python_port():
    ...
    py_q = onnxsim.apply_gguf_q2_k_quantization(model)
    cpp_q = onnxsim.apply_gguf_q2_k_quantization_cpp(model)
...
def test_cpp_ragged_last_super_block_matches_python_zero_padding():
    # K not a multiple of 256 -- passes/gguf_q2_k.h's own scope note
    # claims this ragged-last-super-block approach is mathematically
    # identical to gguf_q2_k.py's zero-pad-then-discard one, since padding
    # zeros can never change a sub-block's own min/max. Checked directly
    # here.
...
def test_cpp_noop_when_no_matmul_gemm_present():
    ...
    result = onnxsim.apply_gguf_q2_k_quantization_cpp(model)
