"""Tests for ``onnxsim.apply_gguf_q5_0_quantization_cpp``/
``apply_gguf_q5_1_quantization_cpp`` -- the C++-backed ports of
``onnxsim.apply_gguf_q5_0_quantization``/``apply_gguf_q5_1_quantization``
(see ``onnxsim/passes/gguf_legacy_quant_5bit.h``). Like Q4_0/Q4_1, these
formats have no accumulation or iterative-refinement step at all (see that
...
import onnxsim
from onnxsim.gguf_legacy_quant_5bit import _BLOCK_SIZE
...
@pytest.mark.parametrize(
    "apply_fn",
    [
        onnxsim.apply_gguf_q5_0_quantization_cpp,
        onnxsim.apply_gguf_q5_1_quantization_cpp,
    ],
)
...
def test_cpp_at_most_32_distinct_levels_per_32_element_block(apply_fn):
    ...
    # (see passes/gguf_legacy_quant.h's own scope note, reused unchanged by
    # gguf_legacy_quant_5bit.h), i.e. contiguous groups of 32 along the
    # flattened [K, N] buffer -- with 5-bit codes, up to 32 distinct levels.
...
def test_cpp_q5_0_is_symmetric_zero_maps_to_zero():
    ...
    q = onnxsim.apply_gguf_q5_0_quantization_cpp(model)
    new_w = _current_weight(q)
    assert new_w[0, 0] == 0.0


def test_cpp_q5_1_beats_q5_0_on_a_large_constant_offset_weight():
    rng = np.random.default_rng(3)
    K, N = _BLOCK_SIZE * 4, 4
    w = (rng.standard_normal((K, N)) * 0.1 + 1000.0).astype(np.float32)
    model = _matmul_model(w, K, N)

    q5_0 = onnxsim.apply_gguf_q5_0_quantization_cpp(model)
    q5_1 = onnxsim.apply_gguf_q5_1_quantization_cpp(model)
    w64 = w.astype(np.float64)
    err_q5_0 = float(np.mean((w64 - _current_weight(q5_0).astype(np.float64)) ** 2))
    err_q5_1 = float(np.mean((w64 - _current_weight(q5_1).astype(np.float64)) ** 2))
    assert err_q5_1 < err_q5_0
...
def test_cpp_q5_beats_q4_reconstruction_error(apply_fn):
    # A strictly finer code grid (5-bit vs 4-bit) can only shrink
    # reconstruction error for the same block statistics -- cross-checks
    # this port against the already-verified Q4 port on the same input.
    ...
    q5 = apply_fn(model)
    q4 = (
        onnxsim.apply_gguf_q4_0_quantization_cpp(model)
        if "q5_0" in apply_fn.__name__
        else onnxsim.apply_gguf_q4_1_quantization_cpp(model)
    )
    w64 = w.astype(np.float64)
...
def test_cpp_behaves_similarly_to_python_port(apply_fn, py_fn):
    ...
    # passes/gguf_legacy_quant_5bit.h's own documented divergence note), but
...
def test_cpp_ragged_last_block_matches_python_zero_padding(apply_fn, py_fn):
    # K not a multiple of 32 -- gguf_legacy_quant_5bit.h's own scope note
    # (inherited from gguf_legacy_quant.h) claims this ragged-last-block
    # approach is mathematically identical to
    # gguf_legacy_quant_5bit.py's zero-pad-then-discard one, since padding
    # zeros can never change a block's own min/max statistics. Checked
    # directly here.
