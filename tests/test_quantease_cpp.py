"""Tests for ``onnxsim.apply_quantease_cpp`` -- the C++-backed port of
``onnxsim.apply_quantease`` (QuantEase's cyclic coordinate-descent INT4
rounding, see ``onnxsim/quantease.py``). Like ``tests/test_gptq_cpp.py``,
this runs the float model over real calibration data through a real
``onnxruntime``-backed executor -- never a fake/mock executor -- and checks
exact (bit-for-bit) agreement against the pure-Python reference:
``quantease_entry.h`` documents no accepted numerical-algorithm divergence
(no matrix inversion/factorization at all, unlike GPTQ's own Cholesky
step), so both sides should agree exactly modulo ordinary double-precision
summation-order noise, which these fixtures never trigger.
"""
...
def _assert_exact_parity(float_model, calibration_data, **kwargs):
    ...
    py = apply_quantease(float_model, quant, calibration_data, **kwargs)
    cpp = apply_quantease_cpp(float_model, quant, calibration_data, **kwargs)
...
def test_quantease_cpp_matches_python_exactly():
    _assert_exact_parity(_matmul_model(), _correlated_calibration())


def test_quantease_cpp_matches_python_across_shapes_and_epochs():
    for K, N, seed, epochs in [
        (128, 32, 5, 1),
        (256, 64, 11, 8),
        (96, 24, 21, 4),
        (32, 8, 23, 2),
        ...
        _assert_exact_parity(model, cals, num_epochs=epochs)


def test_quantease_cpp_gemm_transb():
...
def test_quantease_cpp_dead_input_channel():
    # A channel with zero variance in the calibration data (H's whole
    # row/column at that index is exactly 0) must not blow up (NaN/inf) the
    # coordinate-descent update on either side, and both sides must still
    # agree exactly.
    K, N = 32, 8
...
def test_quantease_cpp_biased_gemm():
...
def test_quantease_cpp_skips():
    model = _matmul_model()
    quant = onnxsim.quantize_weight_only_int4(model)
    for fn in (apply_quantease, apply_quantease_cpp):
        out = fn(model, quant, [])
        assert [n.op_type for n in out.graph.node] == ["DequantizeLinear", "MatMul"]


def test_quantease_cpp_beats_round_to_nearest_via_onnxruntime():
    model = _matmul_model(K=64, N=16, seed=16)
    x = _correlated_calibration(K=64, num_samples=64, seed=17)[0]["X"]
    quant = onnxsim.quantize_weight_only_int4(model)
    cpp = apply_quantease_cpp(model, quant, [{"X": x}])
    py = apply_quantease(model, quant, [{"X": x}])
    onnx.checker.check_model(cpp)
