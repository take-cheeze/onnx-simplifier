"""Tests for ``onnxsim.apply_gguf_q4_k_quantization_cpp`` -- the C++-backed
port of ``onnxsim.apply_gguf_q4_k_quantization`` (see
``onnxsim/passes/gguf_q4_k.h``). Like the other simple block-quant formats
...
import onnxsim
from onnxsim.gguf_kquant import _SUB_BLOCK_SIZE, _SUPER_BLOCK_SIZE
...
def test_cpp_replaces_weight_with_same_shape_float():
    ...
    q = onnxsim.apply_gguf_q4_k_quantization_cpp(model)
...
def test_cpp_at_most_16_distinct_levels_per_32_element_sub_block():
...
def test_cpp_beats_naive_single_scale_int4_on_mixed_sub_blocks():
    # Q4_K's per-sub-block (scale, min) should beat a single super-block-
    # wide symmetric int4 scale on data whose sub-blocks have wildly
    ...
    q4_k_mse = float(np.mean((flat_in - new_w.reshape(-1)) ** 2))
...
def test_cpp_behaves_similarly_to_python_port():
    ...
    py_q = onnxsim.apply_gguf_q4_k_quantization(model)
    cpp_q = onnxsim.apply_gguf_q4_k_quantization_cpp(model)
...
def test_cpp_ragged_last_super_block_matches_python_zero_padding():
    # K not a multiple of 256 -- passes/gguf_q4_k.h's own scope note
    # claims this ragged-last-super-block approach is mathematically
    # identical to onnxsim.gguf_kquant's zero-pad-then-discard one, since
...
def test_cpp_noop_when_no_matmul_gemm_present():
    ...
    result = onnxsim.apply_gguf_q4_k_quantization_cpp(model)
