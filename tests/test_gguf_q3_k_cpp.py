"""Tests for ``onnxsim.apply_gguf_q3_k_quantization_cpp`` -- the C++-backed
port of ``onnxsim.apply_gguf_q3_k_quantization`` (see
``onnxsim/passes/gguf_q3_k.h``). Like the other simple block-quant formats
...
(``tests/test_gguf_q6_k_cpp.py``, ``tests/test_gguf_q2_k_cpp.py``).
"""
...
import onnxsim
from onnxsim.gguf_q3_k import _CODE_MAX, _CODE_MIN, _SUB_BLOCK_SIZE, _SUPER_BLOCK_SIZE
...
def test_cpp_replaces_weight_with_same_shape_float():
    ...
    q = onnxsim.apply_gguf_q3_k_quantization_cpp(model)
...
def test_cpp_at_most_8_distinct_levels_per_16_element_sub_block():
    # The format's own asymmetric 3-bit element code range ({-4, ..., 3})
    # has 8 distinct values -- see this module's docstring.
    ...
    for start in range(0, flat.size, _SUB_BLOCK_SIZE):
        ...
        assert len(np.unique(block)) <= _CODE_MAX - _CODE_MIN + 1


def test_cpp_beats_naive_single_scale_int3_on_mixed_sub_blocks():
    # Q3_K's per-sub-block scale should beat a single super-block-wide
    # scale on data whose sub-blocks have wildly different magnitudes.
    ...
    for start in range(0, flat_in.size, _SUPER_BLOCK_SIZE):
        ...
        scale = max(np.abs(block).max(), 1e-12) / 4.0
        codes = np.clip(np.round(block / scale), -4, 3)
        naive_out[start : start + _SUPER_BLOCK_SIZE] = codes * scale

    q3_k_mse = float(np.mean((flat_in - new_w.reshape(-1)) ** 2))
...
def test_cpp_worse_reconstruction_than_q6_k_on_same_weight():
    # Q3_K's own asymmetric 3-bit element code is a strictly coarser
    # budget than Q6_K's own 6-bit one -- cross-checks this port against
    # the already-verified Q6_K port on the same input.
    rng = np.random.default_rng(9)
    K, N = _SUPER_BLOCK_SIZE * 2, 8
    w = rng.standard_normal((K, N)).astype(np.float32) * 0.5
    model = _matmul_model(w, K, N)

    q3 = onnxsim.apply_gguf_q3_k_quantization_cpp(model)
    q6 = onnxsim.apply_gguf_q6_k_quantization_cpp(model)
    w64 = w.astype(np.float64)
    err_q3 = np.linalg.norm(w64 - _current_weight(q3).astype(np.float64))
    err_q6 = np.linalg.norm(w64 - _current_weight(q6).astype(np.float64))
    assert err_q3 > err_q6
...
def test_cpp_behaves_similarly_to_python_port():
    ...
    py_q = onnxsim.apply_gguf_q3_k_quantization(model)
    cpp_q = onnxsim.apply_gguf_q3_k_quantization_cpp(model)
...
def test_cpp_ragged_last_super_block_matches_python_zero_padding():
    # K not a multiple of 256 -- passes/gguf_q3_k.h's own scope note
    # claims this ragged-last-super-block approach is mathematically
    # identical to gguf_q3_k.py's zero-pad-then-discard one, since padding
...
def test_cpp_noop_when_no_matmul_gemm_present():
    ...
    result = onnxsim.apply_gguf_q3_k_quantization_cpp(model)
