#!/usr/bin/env python3
"""Run the TinyEngine coverage heuristic over the model suite and report.
...
Like ``scripts/edgeai/run_tidl_compat.py``, this needs no vendor package or
device -- see ``tinyengine_backend.py`` -- so there is no ``--require-*``
flag or ``skipped`` status: it always runs.

Usage:
    run_tinyengine_compat.py --output tinyengine-compat.csv
    run_tinyengine_compat.py --models conv_bn_relu mobilenet_dw_block
"""
...
def main() -> int:
    ...
    ap.add_argument("--output", default="tinyengine-compat.csv")
    args = ap.parse_args()

    selected = args.models or models.names()
    print(
        f"TinyEngine coverage check | {len(selected)} models",
        flush=True,
    )
    ...
    for i, name in enumerate(selected, 1):
        ...
        if r.get("coverage_orig") or r.get("coverage_simp"):
            detail += f", tinyengine={r.get('coverage_orig')}->{r.get('coverage_simp')}"
