...
def apply_gguf_q5_0_quantization(
    ...
    (:func:`onnxsim.apply_gguf_q5_0_quantization_cpp`) only when its own
    ...
    if isinstance(model, str):
        model = onnx.load(model, load_external_data=False)
    return apply_gguf_q5_0_quantization_cpp(model)
...
def apply_gguf_q5_1_quantization(
    ...
    Delegates to the verified C++ port
    (:func:`onnxsim.apply_gguf_q5_1_quantization_cpp`) only when its own
    scope exactly covers this call -- see
    :func:`apply_gguf_q5_0_quantization`'s own docstring for why.
    ...
    if isinstance(model, str):
        model = onnx.load(model, load_external_data=False)
    return apply_gguf_q5_1_quantization_cpp(model)
