...
// See foem_entry.h for the full rationale (including why this duplicates
// gptq_entry.cpp's own candidate-matching/tensor-conversion/Hessian
// machinery rather than sharing it) and onnxsim/foem.py for the technique
// this ports.
...
// --- Candidate matching ----------------------------------------------------
//
// Identical to gptq_entry.cpp's own FindInt4MatmulCandidates.
...
//
// Identical to gptq_entry.cpp's own ReadFloatTensor.
...
//
// Identical to gptq_entry.cpp's own CholeskyLower/InverseSPD/
// InverseHessianCholesky.
...
// Round-half-to-even (banker's rounding) -- identical to gptq_entry.cpp's
// own RoundHalfToEven.
...
// FOEM's own per-column search -- transcribed from foem.py's own
// _foem_quantize_columns, itself a near-copy of gptq.py's own
// _gptq_quantize_columns (mirrored exactly in structure/parameters from
// gptq_entry.cpp's own GptqQuantizeColumns) plus one additional term: at
// each column, alongside GPTQ's own second-order (Hessian) rounding-error
// term, also charge forward `foem_beta` times this column's own
// accumulated first-order drift from `w_nk`'s (never-modified) original
// value. `foem_beta == 0.0` makes this byte-identical to plain GPTQ's own
// GptqQuantizeColumns.
std::vector<std::vector<double>> FoemQuantizeColumns(
  ...
  // w_orig is w_nk itself -- never modified, exactly like foem.py's own
  // `w_orig = w_nk` (a plain alias, not a copy, since w_nk is itself never
  // mutated by this function).
  const std::vector<std::vector<double>>& w_orig = w_nk;
  ...
  for (int64_t block_start = 0; block_start < static_cast<int64_t>(k);
    ...
    for (size_t i = 0; i < bs; ++i) {
      ...
      for (size_t r = 0; r < n; ++r) {
        ...
        const double second_order_err = (w_col - code * s) / d;
        ...
        const double drift =
            (w_col - w_orig[r][static_cast<size_t>(k_abs)]) / d;
        const double err = second_order_err - foem_beta * drift;
        err1[r][i] = err;
...
//
// Identical to gptq_entry.cpp's own ActivationRows/AccumulateActivationRows.
    ...
    for (const auto& gi : graph_inputs) {
      ...
      if (it == batch.end()) {
        throw std::invalid_argument(
            "ApplyFoem: calibration batch is missing "
...
onnx::ModelProto ApplyFoem(
    ...
    const std::vector<std::unordered_map<std::string, onnx::TensorProto>>&
        calibration_data,
    double percdamp, int64_t proc_block_size, double foem_beta) {
  std::unordered_map<std::string, int> q_init_index;
  std::unordered_map<std::string, int> f_init_index;
  ...
  for (const auto& c : candidates) {
    ...
    const std::vector<std::vector<double>> codes_nk =
        FoemQuantizeColumns(w_nk, scale_blocks, c.block_size, h, percdamp,
                            proc_block_size, foem_beta);
