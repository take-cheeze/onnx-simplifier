...
// See gptq_entry.h for the full rationale (including why this follows
...
// --- Candidate matching ----------------------------------------------------
//
// Transcribed from adaround.py's own _find_int4_matmul_candidates (which
// apply_gptq reuses verbatim): joins the float and quantized models by
// node output tensor name. Output-name maps are last-wins with
// first-seen iteration order, mirroring `_node_outputs`' own
// `m[n.output[0]] = n` reassignment exactly.
...
//
// Hand-written scalar kernels (no BLAS/LAPACK dependency), mirroring
// structured_pruning_entry.cpp's own CholeskyLower/InvertLowerTriangular
// approach for that file's SparseGPT port rather than introducing one:
// the dense inverse and Cholesky factorization at GPTQ's heart are small
// (K x K, K the weight's own reduction dim) and run once per layer. As
// the header's own scope note says, results can differ from LAPACK in
// the last ulp or two -- the sequential rounding below is stable under
// that, with residual flips concentrating on exact rounding ties.

using Matrix = std::vector<std::vector<double>>;

// Lower-triangular Cholesky factor L of a symmetric positive-definite A
// (A == L @ L^T). A is SPD here by construction (see InverseHessian
// Cholesky below); a non-positive pivot from extreme ill-conditioning is
// clamped to a tiny positive rather than failing, since numpy would only
// ever reach for one on the same degenerate input by raising.
...
// factor (forward/back substitution per unit column).
...
// The GPTQ paper's own efficient reformulation (see gptq.py's own
// _inverse_hessian_cholesky, mirrored exactly): dead channels (zero
// diagonal -- no calibration activation ever seen) get a fixed diagonal
// of 1, every diagonal entry is damped, and the returned factor U is
// upper-triangular with inverse(h) == U^T @ U.
Matrix InverseHessianCholesky(const Matrix& h, double percdamp) {
  ...
  // Transpose to upper-triangular (mirrors numpy cholesky(...).T).
...
// (and the integer-code decision in _gptq_quantize_columns): ties go to
// the even neighbor, unlike std::round's half-away-from-zero.
...
// GPTQ's sequential, Hessian-compensated integer-code search -- mirrors
// gptq.py's own _gptq_quantize_columns scalar-loop-for-scalar-loop:
// process input channels left to right in proc_block_size-wide blocks,
// quantizing each column to its nearest grid point (reusing the
// precomputed per-block scales, which never change) and charging its
// rounding error to every not-yet-quantized channel through the
// Cholesky-factored inverse Hessian. w_nk/scales_in are [N, K] /
// [N, K/block]; returns the [N, K] code matrix (float64, integral
// values in [-7, 7]).
...
// Same low-nibble-first packing as adaround.py's own _pack_int4 (which
// itself mirrors weight_only_quantize_int4_matmul.h): byte[i] =
// (code[2i] & 0xF) | ((code[2i+1] & 0xF) << 4). Codes are integral
// doubles in [-7, 7]; the & 0xF on the two's-complement truncation
// matches numpy's own `astype(int64) & 0xF` exactly.
...
//
// Probes the float model exactly like every other calibration-driven
// pass (probe-injection/batch-iteration/DLPack-crossing), but accumulates
// what gptq.py's own _activation_rows needs instead of a statistic:
// every observed 2-D-or-higher FLOAT32 activation flattened to rows
// (`reshape(-1, K)`, exact, not an approximation) and concatenated along
// rows across batches. Rank < 2 resolves to no rows at all.
    ...
    for (const auto& gi : graph_inputs) {
      ...
      if (it == batch.end()) {
        throw std::invalid_argument(
            "ApplyGptq: calibration batch is missing "
...
onnx::ModelProto ApplyGptq(
  ...
  // one copy-and-rewrite pass -- mirrors the reference's own
  // `optimized` dict plus final initializer loop exactly.
  ...
  for (const auto& c : candidates) {
    ...
    if (k % c.block_size != 0) {
      continue;  // Outside the scheme's own K-multiple-of-block_size
                 // precondition (quantize_weight_only_int4 only ever
                 // emits those); the group index below would run past
                 // the scale blocks otherwise.
    }
    ...
    // Scale blocks [N, K/block] (transposed back when the stored layout
    // is [K/block, N]) -- mirrors `scale_blocks = scale[.T]` exactly.
    ...
    // `codes_orig = codes_nk[.T]` plus `_pack_int4` exactly. (K a
    // multiple of the even block size, so the code count is always even
    // and no odd-count padding applies -- same precondition as the
    // reference; guarded rather than assumed.)
