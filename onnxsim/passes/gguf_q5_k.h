...
// llama.cpp's GGUF "Q5_K" K-quant format -- C++ port of
// onnxsim.gguf_q5_k's own apply_gguf_q5_k_quantization. See that
...
// DequantizeQ5_KBlock/GetScaleMinK4): a 256-element super-block split
// into 8 sub-blocks of 32 elements, each sub-block getting its own
// asymmetric affine quantizer (``dequant = q * sub_scale + sub_min``)
// whose element code ``q`` is 5 bits ([0, 31]); each sub-block's own
...
// values (d/dmin) -- ``value = d * sc_j * q - dmin * m_j``. Identical to
// gguf_q4_k.h's own Q4_K port except for the element code width (5 bits
// here, vs Q4_K's 4) -- see that header for the shared sub-block/
// super-block geometry this format reuses unchanged.
//
...
//                                   super-block Q5_K quantize-dequantize
//                                   round trip
//
...
// float32 tensor -- the same scope gguf_q6_k.h/gguf_q4_k.h use. Unlike
// onnxsim.gguf_q5_k's own apply_gguf_q5_k_quantization, this port does
...
// storage, exactly like gguf_q6_k.h's/gguf_q4_k.h's own block layout --
...
// min/max here too.
//
// ACCEPTED, PERMANENT DIVERGENCE FROM onnxsim.gguf_q5_k: as with
// gguf_q4_k.h, this scheme has no accumulation/iterative-refinement step
...
// apply_gguf_q5_k_quantization and its _cpp counterpart remain
...
constexpr int64_t kMaxCode = 31;          // 5-bit element code range [0, 31]
...
// reproducing the same precision loss real Q5_K's d/dmin fields carry --
// mirrors onnxsim.gguf_q5_k's own
// `.astype(np.float16).astype(np.float64)` step. Reuses this repo's own
// already-verified fp16 codec, the same reuse gguf_q6_k.h's/gguf_q4_k.h's
...
// Quantize-dequantize round trip for one Q5_K super-block of `count` (<=
// kSuperBlockSize) contiguous elements, written back in place. Mirrors
// onnxsim.gguf_q5_k's own _quantize_dequantize_superblocks: for each
// (possibly ragged) 32-element sub-block j, ideal_scale_j =
// max(sub_block) - min(sub_block), clamped to >= 1e-12, / 31;
...
// / sub_scale_j, 0, 31)); dequant = q * sub_scale_j + sub_min_j.
inline void QuantizeDequantizeQ5KSuperBlock(double* data, int64_t count) {
...
// llama.cpp's Q5_K K-quant format -- matches MatMul/vanilla-Gemm the same
// way GgufQ6K/GgufQ4K do, then quantizes the flattened weight
// super-block-by-super-block.
struct GgufQ5K final : public PredicateBasedPass {
  explicit GgufQ5K()
      : PredicateBasedPass(PassType::Other, PassEfficiency::Complete,
                           PassOptimizationType::Compute) {}
  std::string getPassName() const override { return "gguf_q5_k"; }
  ...
  bool runTransform(Node* n, Graph& graph,
    ...
    for (int64_t start = 0; start < numel;
         start += gguf_q5_k_detail::kSuperBlockSize) {
      const int64_t count =
          std::min(gguf_q5_k_detail::kSuperBlockSize, numel - start);
      gguf_q5_k_detail::QuantizeDequantizeQ5KSuperBlock(out_data.data() + start,
                                                        count);
    }
