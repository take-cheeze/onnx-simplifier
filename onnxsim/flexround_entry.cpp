...
// See flexround_entry.h for the full rationale (including why this
// duplicates adaround_entry.cpp's own candidate-matching/tensor-conversion/
// dense-matmul helpers rather than sharing them, and this being a
// log-space, learnable-division relaxation entirely unlike AdaRound's own
// rectified-sigmoid one) and onnxsim/flexround.py for the technique this
// ports.
...
//
// Identical to adaround_entry.cpp's own ReadFloatTensor.
    ...
    for (const auto& gi : graph_inputs) {
      ...
      if (it == batch.end()) {
        throw std::invalid_argument(
            "ApplyFlexround: calibration batch is missing "
...
// flexround.py's own _optimize_divisor, transcribed scalar-loop-for-
// scalar-loop. `w_nk`/`scale_nk` are [N, K] (output channel first,
// already block-broadcast); returns the [N, K] integer code matrix.
Matrix OptimizeDivisor(const Matrix& w_nk, const Matrix& scale_nk,
  ...
  std::vector<double> v3(n_rows, 0.0);  // [N, 1], stored flat.

  Matrix m2(n_rows, std::vector<double>(k, 0.0));
  Matrix u2(n_rows, std::vector<double>(k, 0.0));
  std::vector<double> m3(n_rows, 0.0);
  std::vector<double> u3(n_rows, 0.0);
  constexpr double kAdamBeta1 = 0.9, kAdamBeta2 = 0.999, kAdamEps = 1e-8;

  for (int64_t t = 0; t < num_iterations; ++t) {
    Matrix s(n_rows, std::vector<double>(k));
    Matrix ratio(n_rows, std::vector<double>(k));
    ...
    for (size_t r = 0; r < n_rows; ++r) {
      const double s3 = std::exp(v3[r]);
      for (size_t c = 0; c < k; ++c) {
        const double s2 = std::exp(v2[r][c]);
        const double s_val = scale_nk[r][c] * s2 * s3;
        s[r][c] = s_val;
        const double rw = w_nk[r][c] / s_val;
        ratio[r][c] = rw;
        const bool act = rw > n_min && rw < n_max;
        active[r][c] = act ? 1 : 0;
        const double code = std::clamp(rw, n_min, n_max);
        codes[r][c] = code;
        w_hat[r][c] = code * scale_nk[r][c];
      }
    }
    ...
    for (size_t s_idx = 0; s_idx < num_samples; ++s_idx) {
      for (size_t r = 0; r < n_rows; ++r) {
        dl_dy[s_idx][r] = 2.0 * (y_hat[s_idx][r] - y_float[s_idx][r]) / n_elems;
      }
    }
    const Matrix dl_dw_hat = DlDwHatFromDlDyX(dl_dy, x);

    Matrix grad_v2(n_rows, std::vector<double>(k));
    std::vector<double> grad_v3(n_rows, 0.0);
    for (size_t r = 0; r < n_rows; ++r) {
      for (size_t c = 0; c < k; ++c) {
        const double dw_hat_dratio = active[r][c] ? scale_nk[r][c] : 0.0;
        const double dratio_ds =
            active[r][c] ? (-w_nk[r][c] / (s[r][c] * s[r][c])) : 0.0;
        const double grad_s = dl_dw_hat[r][c] * dw_hat_dratio * dratio_ds;
        const double grad_sv = grad_s * s[r][c];
        grad_v2[r][c] = grad_sv;
        grad_v3[r] += grad_sv;
      }
    }
    ...
    for (size_t r = 0; r < n_rows; ++r) {
      for (size_t c = 0; c < k; ++c) {
        m2[r][c] = kAdamBeta1 * m2[r][c] + (1.0 - kAdamBeta1) * grad_v2[r][c];
        u2[r][c] = kAdamBeta2 * u2[r][c] +
                   (1.0 - kAdamBeta2) * grad_v2[r][c] * grad_v2[r][c];
        v2[r][c] -= learning_rate * (m2[r][c] / bias_c1) /
                    (std::sqrt(u2[r][c] / bias_c2) + kAdamEps);
        v2[r][c] = std::clamp(v2[r][c], -log_clip, log_clip);
      }
      m3[r] = kAdamBeta1 * m3[r] + (1.0 - kAdamBeta1) * grad_v3[r];
      u3[r] = kAdamBeta2 * u3[r] + (1.0 - kAdamBeta2) * grad_v3[r] * grad_v3[r];
      v3[r] -= learning_rate * (m3[r] / bias_c1) /
               (std::sqrt(u3[r] / bias_c2) + kAdamEps);
      v3[r] = std::clamp(v3[r], -log_clip, log_clip);
    }
  }

  Matrix codes_final(n_rows, std::vector<double>(k));
  for (size_t r = 0; r < n_rows; ++r) {
    const double s3 = std::exp(v3[r]);
    for (size_t c = 0; c < k; ++c) {
      const double s2 = std::exp(v2[r][c]);
      const double s_final = scale_nk[r][c] * s2 * s3;
      codes_final[r][c] =
          std::clamp(RoundHalfToEven(w_nk[r][c] / s_final), n_min, n_max);
    }
  }
  return codes_final;
}
...
onnx::ModelProto ApplyFlexround(
    ...
    const std::vector<std::unordered_map<std::string, onnx::TensorProto>>&
        calibration_data,
    int64_t num_iterations, double learning_rate, double log_clip) {
  std::unordered_map<std::string, int> q_init_index;
  std::unordered_map<std::string, int> f_init_index;
  ...
  for (const auto& c : candidates) {
    ...
    const Matrix codes_nk =
        OptimizeDivisor(w_nk, scale_nk, x, kNMin, kNMax, num_iterations,
                        learning_rate, log_clip);
